#define _WINSOCK_DEPRECATED_NO_WARNINGS
#define recv_s(Socket,DataPtr,Size,Other) recv( (Socket), (char *)(DataPtr), (Size), (Other))
#define send_s(Socket,DataPtr,Size,Other) send( (Socket), (char *)(DataPtr), (Size), (Other)) 
#include <winsock2.h>
#pragma comment(lib,"ws2_32.lib")
#include "../DubuEngineServer/DEProtocol.hpp"

#include <process.h>
#include <fstream>
#include <sstream>
#include <allegro5/allegro.h>
#include <allegro5/allegro_font.h>
#include <allegro5/allegro_ttf.h>
#include <allegro5/allegro_native_dialog.h>
#include <allegro5/allegro_primitives.h>
#include <allegro5/allegro_image.h>
#include <stdlib.h>
#include <time.h>
#include <allegro5/allegro_audio.h>
#include <allegro5/allegro_acodec.h>
#include <string.h>
#include <iostream>
#include "bass.h"
#include "DEEvents.h"
#include "Main.hpp"
#include "Game.hpp"
#include "MenuRenderer.hpp"
#include "MenuHandler.hpp"
#include "GameHandler.hpp"
#include "GameRenderer.hpp"
#include "InputHandler.hpp"
#include "Interfaces.hpp"
#include "Physics.h"
#include "DEText.h"


#include "Item.h"

int main(){
	// Initiate Allegro
	if(!al_init()){
		al_show_native_message_box(NULL, NULL, NULL, "failed to initialize allegro!", NULL, NULL);                   
		return -1;
	}

	// Initiate BASS
	if (BASS_Init(-1, 44100, 0, 0, NULL)) {
		menu_theme = BASS_SampleLoad(false, "music/theme_0.mp3", 0, 0, 1, BASS_SAMPLE_LOOP);
		bg_theme = BASS_SampleLoad(false, "music/bg_0.mp3", 0, 0, 1, BASS_SAMPLE_LOOP);
		level_theme = BASS_SampleLoad(false, "music/searching.mp3", 0, 0, 1, BASS_SAMPLE_LOOP);
		main_channel = BASS_SampleGetChannel(menu_theme, false);
		bg_channel = BASS_SampleGetChannel(bg_theme, false);
		level_channel = BASS_SampleGetChannel(level_theme, false);
	} else {
		al_show_native_message_box(NULL, NULL, NULL, "failed to initialize Bass", NULL, NULL);
	}

	// Load settings
	LoadSettings();

	// Initiate display
	al_set_new_display_flags(g.window_mode);
	display = al_create_display(g.s_x, g.s_y);
	if (!display)
	{
		al_show_native_message_box(NULL, NULL, NULL, "failed to initialize display!", NULL, NULL);
		return -1;
	}

	// Set scene to -1 (loading)
	g.scene = -1;

	// Set random seed
	srand(time(NULL));

	// Register event functions
	RegisterNPCEvents(TimerEvents, MouseEvents, KeyboardEvents);
	RegisterPhysicsEvents(TimerEvents, MouseEvents, KeyboardEvents);
	RegisterLevelEvents(TimerEvents, MouseEvents, KeyboardEvents);
	RegisterWeatherEvents(TimerEvents, MouseEvents, KeyboardEvents);
	RegisterAnimationEvents(TimerEvents, MouseEvents, KeyboardEvents);
	RegisterItemEvents(TimerEvents, MouseEvents, KeyboardEvents);
	RegisterPlayerEvents(TimerEvents, MouseEvents, KeyboardEvents);
	RegisterCombatEvents(TimerEvents, MouseEvents, KeyboardEvents);
	RegisterConvoEvents(TimerEvents, MouseEvents, KeyboardEvents);
	RegisterCameraEvents(TimerEvents, MouseEvents, KeyboardEvents);
	RegisterExperienceEvents(TimerEvents, MouseEvents, KeyboardEvents);
	RegisterObjectiveInterfaceEvents(TimerEvents, MouseEvents, KeyboardEvents);
	RegisterRewardInterfaceEvents(TimerEvents, MouseEvents, KeyboardEvents);
	RegisterEditorEvents(TimerEvents, MouseEvents, KeyboardEvents);

	// Initiate allegro addons, registed mouse, keyboard, etc.
	al_init_primitives_addon();
	al_init_image_addon();
	al_install_keyboard();
	al_install_mouse();

	// Load Icon
	char_sprites.img_sprite[44] = al_load_bitmap("sprites/sprite_44.png");
	if (char_sprites.img_sprite[44] != NULL) {
		al_set_display_icon(display, char_sprites.img_sprite[44]);
		al_set_window_title(display, ENGINE_VERSION);
	} else {
		g.done = true;
		al_show_native_message_box(display, "Error", "Can't load icon (sprites/sprite_44.png)", "Game will quit.", "Quit", 0);
	}

	// Load samples
	LoadSamples();

	// Load fonts
	LoadFonts();

	// Load/initilizae interfaces
	LoadInterfaces(&g);

	// Initiate client socket
	InitClient();

	// Initiate timer and register events
	timer = al_create_timer(1.0 / TPS);
	event_queue = al_create_event_queue();
	al_register_event_source(event_queue, al_get_keyboard_event_source());
	al_register_event_source(event_queue, al_get_mouse_event_source());
	al_register_event_source(event_queue, al_get_display_event_source(display));
	al_register_event_source(event_queue, al_get_timer_event_source(timer));
	al_hide_mouse_cursor(display);

	// Main Loop
	MainLoop();

	// Save settings
	SaveSettings();

	// Free memory once loop ends
	BASS_Free();
	DestroyBitmaps();
	DestroySamples();
	DestroyFonts();
	al_destroy_event_queue(event_queue);
	al_destroy_timer(timer);
	al_destroy_display(display);
	return 0;
}

// Temp
void BgTheme() {
	if (g.rdrop[0].fall_speed > 0 && BASS_ChannelIsActive(bg_channel) == BASS_ACTIVE_PAUSED) {
		BASS_ChannelPlay(bg_channel, true);
	} else if(g.rdrop[0].fall_speed <= 0 && BASS_ChannelIsActive(bg_channel) == BASS_ACTIVE_PLAYING){
		BASS_ChannelPause(bg_channel);
	}
}
void LevelTheme() {
	if (g.story.missions[0].prog == 1 && BASS_ChannelIsActive(level_channel) == BASS_ACTIVE_PAUSED) {
		BASS_ChannelPlay(level_channel, true);
	} else if (g.scene == 0 && BASS_ChannelIsActive(level_channel) == BASS_ACTIVE_PLAYING) {
		BASS_ChannelPause(level_channel);
	}
}

void LoadGame() {
	RenderLoading(&g, NULL, NULL, font);
	al_flip_display();
	al_clear_to_color(al_map_rgb(0, 0, 0));
	LoadBackgrounds();
	LoadSprites();
	LoadAccessories();
	LoadBodies();
	LoadHairs();
	LoadOutfits();
	LoadTiles();
	LoadIcons();
	g.menu.bg_x = (-1500 + GAME_WIDTH_) + (1500 - GAME_WIDTH_) * (float)((float)(GAME_WIDTH_ - g.pos_x) / (float)GAME_WIDTH_);
	g.menu.bg_y = (-938 + GAME_HEIGHT_) + (938 - GAME_HEIGHT_) * (float)((float)(GAME_HEIGHT_ - g.pos_y) / (float)GAME_HEIGHT_);
	g.scene = 0;
	g.lvl = LoadLevel("maps/Menu.dem");
	al_start_timer(timer);
}

void MainLoop() {
	LoadGame();
	while (!g.done) {
		if (g.remake_display) {
			std::cout << "Recreating Display...\n";
			g.remake_display = false;
			int recreate = false;
			if (al_get_display_flags(display) != g.window_mode) {
				recreate = true;
			}

			// Make sure everything is up to standard
			// 1280x720 is minimum base res, anything bellow that will get downscaled
			if (g.BWIDTH < 1280 || g.BHEIGHT < 720) {
				g.BWIDTH = 1280;
				g.BHEIGHT = 720;
			}
			ALLEGRO_MONITOR_INFO monitor_info;
			al_get_monitor_info(0, &monitor_info);
			if (g.window_mode == ALLEGRO_WINDOWED) {
				if (g.s_x > monitor_info.x2) {
					g.s_x = monitor_info.x2;
					g.BWIDTH = monitor_info.x2;
					g.s_y = (int)((monitor_info.x2 / 16) * 9);
					g.BHEIGHT = (int)((monitor_info.x2 / 16) * 9);
				}
			}
			if (g.window_mode == ALLEGRO_FULLSCREEN_WINDOW) {
				g.s_x = monitor_info.x2;
				g.s_y = monitor_info.y2;
				g.BWIDTH = monitor_info.x2;
				g.BHEIGHT = monitor_info.y2;
			}
			if (g.window_mode == ALLEGRO_FULLSCREEN) {
				if (g.s_x > monitor_info.x2 || g.s_y > monitor_info.y2) {
					g.s_x = monitor_info.x2;
					g.s_y = monitor_info.y2;
					g.BWIDTH = monitor_info.x2;
					g.BHEIGHT = monitor_info.y2;
				}
			}

			// Recreate fonts according to new resolution
			DestroyFonts();
			LoadFonts();
			if (g.window_mode == ALLEGRO_WINDOWED && !recreate) {
				al_resize_display(display, g.s_x, g.s_y);
				al_toggle_display_flag(display, ALLEGRO_FULLSCREEN_WINDOW, false);
			} else if (g.window_mode == ALLEGRO_FULLSCREEN_WINDOW && !recreate) {
				al_resize_display(display, g.s_x, g.s_y);
				al_toggle_display_flag(display, ALLEGRO_FULLSCREEN_WINDOW, true);
			} else {
				// Prevent memory leaks
				DestroyBitmaps();
				DestroySamples();
				al_destroy_display(display);
				al_destroy_timer(timer);
				al_destroy_event_queue(event_queue);

				// Create new display
				al_set_new_display_flags(g.window_mode);
				display = al_create_display(g.s_x, g.s_y);

				RenderLoading(&g, NULL, NULL, font);
				al_flip_display();
				al_clear_to_color(al_map_rgb(0, 0, 0));

				LoadSprites();
				al_set_display_icon(display, char_sprites.img_sprite[44]);
				al_set_window_title(display, ENGINE_VERSION);
				LoadBackgrounds();
				LoadSamples();
				LoadAccessories();
				LoadBodies();
				LoadHairs();
				LoadOutfits();
				LoadTiles();
				LoadIcons();
				timer = al_create_timer(1.0 / 120);
				event_queue = al_create_event_queue();
				al_register_event_source(event_queue, al_get_keyboard_event_source());
				al_register_event_source(event_queue, al_get_mouse_event_source());
				al_register_event_source(event_queue, al_get_display_event_source(display));
				al_register_event_source(event_queue, al_get_timer_event_source(timer));
				al_start_timer(timer);
				al_hide_mouse_cursor(display);
			}
		}
		ALLEGRO_EVENT ev;
		al_wait_for_event(event_queue, &ev);
		if (ev.type == ALLEGRO_EVENT_DISPLAY_SWITCH_IN && g.window_mode == ALLEGRO_FULLSCREEN) {
			g.remake_display = true;
		}
		if (ev.type == ALLEGRO_EVENT_DISPLAY_CLOSE)
		{
			g.done = true;
		}
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			if (g.scene != -1) {
				BgTheme();		// Temp
				LevelTheme();	// Temp

				HandleInterfaces(&g);
				// Timer values
				g.old_time = g.new_time;
				g.new_time = al_get_time();

				if (g.logini.try_login == true) {
					g.logini.try_login = false;
					Connect(&g);
				}
			}
			if (g.scene == -1) {
				// Loading
			} else if (g.scene == 0) { // Menu
				HandleMenu(&g, sample_sfx);
				if(g.scene == 0) RenderMenu(&g, &char_sprites, font);
				if (g.intro_a == 0) RenderInterfaces(&g, &char_sprites, font);
				al_flip_display();
				al_clear_to_color(al_map_rgb(0, 0, 0));
			} else if (g.scene == 1) { // Game
				// Tick
				for (int i = 0; i < MAX_EVENT_FUNCTIONS; i++) {
					if (TimerEvents[i] != NULL) {
						TimerEvents[i](&g, sample_sfx);
					}
				}
				HandleGame(&g, sample_sfx);
				RenderGame(&g, &char_sprites, font);
				RenderInterfaces(&g, &char_sprites, font);
				al_flip_display();
				al_clear_to_color(al_map_rgb(10, 10, 10));
			} else if (g.scene == 2) { // Pause
				RenderGame(&g, &char_sprites, font);
				RenderInterfaces(&g, &char_sprites, font);
				al_flip_display();
				al_clear_to_color(al_map_rgb(10, 10, 10));
			}
		} else if (ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN) {
			for (int i = 0; i < MAX_EVENT_FUNCTIONS; i++) {
				if (MouseEvents[i] != NULL && g.allow_input == true) {
					MouseEvents[i](&g, ev.mouse.button, false, sample_sfx);
				}
			}
			if (g.allow_input) {
				if (ev.mouse.button == 1) {
					LeftClick(&g, false, sample_sfx);
				} else if (ev.mouse.button == 2) {
					RightClick(&g, false, sample_sfx);
				}
			}
			g.allow_input = true;
		} else if (ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP) {
			for (int i = 0; i < MAX_EVENT_FUNCTIONS; i++) {
				if (MouseEvents[i] != NULL && g.allow_input == true) {
					MouseEvents[i](&g, ev.mouse.button, true, sample_sfx);
				}
			}
			if (g.allow_input) {
				if (ev.mouse.button == 1) {
					LeftClick(&g, true, sample_sfx);
				} else if (ev.mouse.button == 2) {
					RightClick(&g, true, sample_sfx);
				}
			}
			g.allow_input = true;
		} else if (ev.type == ALLEGRO_EVENT_MOUSE_AXES) {
			float r_x = (float)GAME_WIDTH_ / (float)g.s_x;
			float r_y = (float)GAME_HEIGHT_ / (float)g.s_y;
			g.pos_x = (int)((r_x)*ev.mouse.x);
			g.pos_y = (int)((r_y)*ev.mouse.y);
			if (ev.mouse.dz > 0) {
				if (g.Interfaces[INTERFACE_EDITOR_DEBUG].visible) {
					g.Editor.debug_scroll--;
					if (g.keys.CTRL) g.Editor.debug_scroll -= 27;
					if (g.Editor.debug_scroll < 0) g.Editor.debug_scroll = 0;
				} else {
					g.music_volume += 0.1;
					if (g.music_volume > 1) g.music_volume = 1;
					g.music_volume_cd = 240;
					g.Interfaces[7].visible = true;
					BASS_ChannelSetAttribute(main_channel, BASS_ATTRIB_VOL, g.music_volume);
				}
			} else if (ev.mouse.dz < 0) {
				if (g.Interfaces[INTERFACE_EDITOR_DEBUG].visible) {
					g.Editor.debug_scroll++;
					if (g.keys.CTRL) g.Editor.debug_scroll += 27;
					if (g.Editor.debug_scroll > 1000) g.Editor.debug_scroll = 1000;
				} else {
					g.music_volume -= 0.1;
					if (g.music_volume < 0) g.music_volume = 0;
					g.music_volume_cd = 240;
					g.Interfaces[7].visible = true;
					BASS_ChannelSetAttribute(main_channel, BASS_ATTRIB_VOL, g.music_volume);
				}
			}
		} else if (ev.type == ALLEGRO_EVENT_KEY_DOWN) {
			for (int i = 0; i < MAX_EVENT_FUNCTIONS; i++) {
				if (KeyboardEvents[i] != NULL && g.allow_input == true) {
					KeyboardEvents[i](&g, ev.keyboard.keycode, false, sample_sfx);
				}
			}
			if(g.allow_input == true) KeyboardFunction(&g, ev.keyboard.keycode, false, sample_sfx, font);
			g.allow_input = true;
		} else if (ev.type == ALLEGRO_EVENT_KEY_UP) {
			KeyboardFunction(&g, ev.keyboard.keycode, true, sample_sfx);
		} else if (ev.type == ALLEGRO_EVENT_DISPLAY_CLOSE) {
			g.done = true;
		}
	}
}

int LoadSprites(){
	std::cout << "Loading sprites..." << "\n";
	for(int i = 0; i < MAX_SPRITES; i++){
		std::stringstream strs;
		strs << "sprites/sprite_";
		strs << i;
		strs << ".png";
		std::string temp_str = strs.str();
		char const* n = temp_str.c_str();
		std::cout << n << "\n";
		char_sprites.img_sprite[i] = al_load_bitmap(n);
		if (char_sprites.img_sprite[i] == NULL) {
			std::cout << "Can't access file: " << n << "\n";
		}
	}
	return 0;
}

int LoadAccessories() {
	std::cout << "Loading accessories..." << "\n";
	for (int i = 0; i < MAX_ACCESSORY; i++) {
		std::stringstream strs;
		strs << "sprites/accessory/accessory_";
		strs << i;
		strs << ".png";
		std::string temp_str = strs.str();
		char const* n = temp_str.c_str();
		std::cout << n << "\n";
		char_sprites.img_acc[i] = al_load_bitmap(n);
		if (char_sprites.img_acc[i] == NULL) {
			std::cout << "Can't access file: " << n << "\n";
		}
	}
	return 0;
}

int LoadBodies() {
	std::cout << "Loading bodies..." << "\n";
	for (int i = 0; i < MAX_BODY; i++) {
		std::stringstream strs;
		strs << "sprites/body/body_";
		strs << i;
		strs << ".png";
		std::string temp_str = strs.str();
		char const* n = temp_str.c_str();
		std::cout << n << "\n";
		char_sprites.img_body[i] = al_load_bitmap(n);
		if (char_sprites.img_body[i] == NULL) {
			std::cout << "Can't access file: " << n << "\n";
		}
	}
	return 0;
}

int LoadHairs() {
	std::cout << "Loading hairstyles..." << "\n";
	for (int i = 0; i < MAX_HAIR; i++) {
		std::stringstream strs;
		strs << "sprites/hair/hair_";
		strs << i;
		strs << ".png";
		std::string temp_str = strs.str();
		char const* n = temp_str.c_str();
		std::cout << n << "\n";
		char_sprites.img_hair[i] = al_load_bitmap(n);
		if (char_sprites.img_hair[i] == NULL) {
			std::cout << "Can't access file: " << n << "\n";
		}
	}
	return 0;
}

int LoadOutfits() {
	std::cout << "Loading outfits..." << "\n";
	for (int i = 0; i < MAX_OUTFIT; i++) {
		std::stringstream strs;
		strs << "sprites/outfit/outfit_";
		strs << i;
		strs << ".png";
		std::string temp_str = strs.str();
		char const* n = temp_str.c_str();
		std::cout << n << "\n";
		char_sprites.img_outfit[i] = al_load_bitmap(n);
		if (char_sprites.img_outfit[i] == NULL) {
			std::cout << "Can't access file: " << n << "\n";
		}
	}
	return 0;
}

int LoadTiles() {
	std::cout << "Loading tiles..." << "\n";
	for (int i = 0; i < MAX_TILES; i++) {
		std::stringstream strs;
		strs << "sprites/tiles/tile_";
		strs << i;
		strs << ".png";
		std::string temp_str = strs.str();
		char const* n = temp_str.c_str();
		std::cout << n << "\n";
		char_sprites.img_tiles[i] = al_load_bitmap(n);
		if (char_sprites.img_tiles[i] == NULL) {
			std::cout << "Can't access file: " << n << "\n";
		}
	}
	return 0;
}

int LoadIcons() {
	std::cout << "Loading icons..." << "\n";
	for (int i = 0; i < MAX_ICONS; i++) {
		std::stringstream strs;
		strs << "sprites/icons/icon_";
		strs << i;
		strs << ".png";
		std::string temp_str = strs.str();
		char const* n = temp_str.c_str();
		std::cout << n << "\n";
		char_sprites.img_icons[i] = al_load_bitmap(n);
		if (char_sprites.img_icons[i] == NULL) {
			std::cout << "Can't access file: " << n << "\n";
		}
	}
	return 0;
}

int LoadBackgrounds(){
	std::cout << "Loading backgrounds..." << "\n";
	for(int i = 0; i < MAX_BACKGROUNDS; i++){
		std::stringstream strs;
		strs << "bg/bg_";
		strs << i;
		strs << ".png";
		std::string temp_str = strs.str();
		char const* n = temp_str.c_str();
		std::cout << n << "\n";
		char_sprites.img_background[i] = al_load_bitmap(n);
		if (char_sprites.img_background[i] == NULL) {
			std::cout << "Can't access file: " << n << "\n";
		}
	}
	return 1;
}

int LoadSamples(){
	std::cout << "Loading samples..." << "\n";
	al_install_audio();
	al_init_acodec_addon();
	al_reserve_samples(10);
	for(int i = 0; i < MAX_SAMPLES; i++){
		std::stringstream strs;
		strs << "sounds/sfx_";
		strs << i;
		strs << ".wav";
		std::string temp_str = strs.str();
		char const* n = temp_str.c_str();
		std::cout << n << "\n";
		sample_sfx[i] = al_load_sample(n);
		if (sample_sfx[i] == NULL) {
			std::cout << "Can't access file: " << n << "\n";
		}
	}
	return 0;
}

int LoadFonts(){
	std::cout << "Loading fonts..." << "\n";
	al_init_font_addon();
	al_init_ttf_addon();
	float r_y = (float)g.s_y/(float)GAME_HEIGHT_;
	font[0] = al_load_font("fonts/Exo2-Bold.ttf", (int)(r_y*24), 0);
	font[1] = al_load_font("fonts/Exo2-Bold.ttf", (int)(r_y*18), 0);
	font[2] = al_load_font("fonts/Exo2-Bold.ttf", (int)(r_y*16), 0);
	font[3] = al_load_font("fonts/Exo2-Bold.ttf", (int)(r_y*20), 0);
	font[4] = al_load_font("fonts/Exo2-Bold.ttf", (int)(r_y*12), 0);
	font[5] = al_load_font("fonts/Exo2-Bold.ttf", 18, 0);				// Constant chat font (for calculations)
	font[6] = al_load_font("fonts/Exo2-Bold.ttf", (int)(r_y * 32), 0);
	font[7] = al_load_font("fonts/Exo2-Bold.ttf", (int)(r_y * 48), 0);
	font[8] = al_load_font("fonts/Exo2-Bold.ttf", (int)(r_y * 64), 0);
	font[9] = al_load_font("fonts/Exo2-Bold.ttf", (int)(r_y * 14), 0);
	return 0;
}

void DestroyBitmaps(){
	std::cout << "Destroying bitmaps..." << "\n";
	for(int i = 0; i < MAX_SPRITES; i++){
		al_destroy_bitmap(char_sprites.img_sprite[i]);
	}
	for(int i = 0; i < MAX_BACKGROUNDS; i++){
		al_destroy_bitmap(char_sprites.img_background[i]);
	}
	for (int i = 0; i < MAX_ACCESSORY; i++) {
		al_destroy_bitmap(char_sprites.img_acc[i]);
	}
	for (int i = 0; i < MAX_BODY; i++) {
		al_destroy_bitmap(char_sprites.img_body[i]);
	}
	for (int i = 0; i < MAX_HAIR; i++) {
		al_destroy_bitmap(char_sprites.img_hair[i]);
	}
	for (int i = 0; i < MAX_OUTFIT; i++) {
		al_destroy_bitmap(char_sprites.img_outfit[i]);
	}
	for (int i = 0; i < MAX_TILES; i++) {
		al_destroy_bitmap(char_sprites.img_tiles[i]);
	}
	for (int i = 0; i < MAX_ICONS; i++) {
		al_destroy_bitmap(char_sprites.img_icons[i]);
	}
}

void DestroySamples(){
	std::cout << "Destroying samples..." << "\n";
	for(int i = 0; i < MAX_SAMPLES; i++){		
		al_destroy_sample(sample_sfx[i]);
	}
}

void DestroyFonts(){	
	std::cout << "Destroying fonts..." << "\n";
	for(int i = 0; i < MAX_FONTS; i++){
		al_destroy_font(font[i]);
	}
}

void LoadSettings(){
	std::ifstream file("settings.ini");

	while (!file.eof() && file.is_open()) {
		std::string meme;
		std::getline(file, meme);
		SubStrings setting = SplitString(meme, '=');

		if (setting.items == 3) {
			std::string setting_type = setting.s[0];
			int s_val = atoi(trim(setting.s[2]).c_str());
			if (setting_type == "BASE_WIDTH") g.BWIDTH = s_val;
			if (setting_type == "BASE_HEIGHT") g.BHEIGHT = s_val;
			if (setting_type == "WINDOW_WIDTH") g.s_x = s_val;
			if (setting_type == "WINDOW_HEIGHT") g.s_y = s_val;
			if (setting_type == "WINDOW_MODE") g.window_mode = s_val;
			for (int i = 0; i < 8; i++) {
				if (setting_type == ("KEY_SKILL_" + std::to_string(i + 1))) g.AbilitySlots[i].key = s_val;
			}
			if (setting_type == ("KEY_LEFT")) g.keys.left_bind = s_val;
			if (setting_type == ("KEY_RIGHT")) g.keys.right_bind = s_val;
			if (setting_type == ("KEY_JUMP")) g.keys.jump_bind = s_val;
			if (setting_type == ("KEY_CROUCH")) g.keys.crouch_bind = s_val;
			if (setting_type == ("KEY_CAMERA")) g.keys.camera_bind = s_val;
			if (setting_type == ("KEY_INVENTORY")) g.keys.inventory_bind = s_val;
			if (setting_type == ("KEY_SKILLS")) g.keys.skills_bind = s_val;
			if (setting_type == ("KEY_CHAT")) g.keys.chat_bind = s_val;
		}
	}
	file.close();

	if (g.s_x > 1920 || g.s_x < 640) {
		g.s_x = GAME_HEIGHT_;
		g.s_y = GAME_WIDTH_;
	}
	if (g.s_y > 1080 || g.s_y < 360) {
		g.s_x = GAME_HEIGHT_;
		g.s_y = GAME_WIDTH_;
	}
	if (g.s_y != (int)((g.s_x / 16) * 9)) {
		g.s_y = (int)((g.s_x / 16) * 9);
	}
	if (g.window_mode != ALLEGRO_FULLSCREEN_WINDOW &&
		g.window_mode != ALLEGRO_WINDOWED &&
		g.window_mode != ALLEGRO_FULLSCREEN) g.window_mode = ALLEGRO_WINDOWED;
}

void SaveSettings(){
	using namespace std;
	cout << "Saving settings..." << "\n";
	ofstream myfile;
	myfile.open ("settings.ini");
	myfile << ("BASE_WIDTH = " + to_string(g.BWIDTH) + "\n");
	myfile << ("BASE_HEIGHT = " + to_string(g.BHEIGHT) + "\n");
	myfile << ("WINDOW_WIDTH = " + to_string(g.s_x) + "\n");
	myfile << ("WINDOW_HEIGHT = " + to_string(g.s_y) + "\n");
	myfile << ("WINDOW_MODE = " + to_string(g.window_mode) + "\n");
	for (int i = 0; i < 8; i++) {
		myfile << ("KEY_SKILL_" + to_string(i + 1) + " = " + to_string(g.AbilitySlots[i].key) + "\n");
	}
	myfile << ("KEY_LEFT = " + to_string(g.keys.left_bind) + "\n");
	myfile << ("KEY_RIGHT = " + to_string(g.keys.right_bind) + "\n");
	myfile << ("KEY_JUMP = " + to_string(g.keys.jump_bind) + "\n");
	myfile << ("KEY_CROUCH = " + to_string(g.keys.crouch_bind) + "\n");
	myfile << ("KEY_CAMERA = " + to_string(g.keys.camera_bind) + "\n");
	myfile << ("KEY_INVENTORY = " + to_string(g.keys.inventory_bind) + "\n");
	myfile << ("KEY_SKILLS = " + to_string(g.keys.skills_bind) + "\n");
	myfile << ("KEY_CHAT = " + to_string(g.keys.chat_bind) + "\n");
	myfile.close();
}

int InitClient() {
	int iResult = WSAStartup(MAKEWORD(2, 2), &WsaDat);

	if (iResult) {
		std::cout << "WSA startup failed\n";
		return 0;
	}

	sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);

	if (sock == INVALID_SOCKET) {
		std::cout << "Invalid Socket\n";
		return 0;
	}
	return 1;
}

void Connect(Game* g) {
	_beginthreadex(0, 0, ClientThread, g, 0, 0);
}

unsigned int __stdcall ClientThread(void *data)
{
	std::cout << "Connecting to server...\n";
	Game* g = (Game*)data;
	sockaddr_in addr;
	addr.sin_family = AF_INET;
	addr.sin_port = htons(g->server_port);
	addr.sin_addr.S_un.S_addr = inet_addr(g->server_ip);

	int iResult = connect(sock, (SOCKADDR*)&addr, sizeof(sockaddr_in));
	if (iResult) {
		std::cout << "Failed to connect: " << WSAGetLastError() << "\n";
		ShowMessage(g, "Failed to connect to server.", "");
		g->logini.logging_in = false;
		g->Buttons[247].visible = true;
		g->connected = false;
		g->scene = 0;
		return 0;
	}
	std::cout << "Successfully connected to " << g->server_ip << "\n";
	connected = true;
	AddPacketToQueue(g, MakePacket(PACKET_TYPE_AUTHENTICATE, g->logini.username_input.c_str(), g->logini.password_input.c_str())); // Authenticate
	g->ReadyToSendPackets = true;
	DEPacket msg = MakePacket(-1, "", "");
	
	// Disable nagle
	int flag = 1;
	setsockopt(sock,		// Socket affected
		IPPROTO_TCP,		// Set option at TCP level
		TCP_NODELAY,		// Name of option
		(char *)&flag,		// The cast is historical cruft
		sizeof(int));		// Length of option value

	while (connected) {
		Sleep(1);
		u_long iMode = 1;
		ioctlsocket(sock, FIONBIO, &iMode);
		int nReadBytes = recv_s(sock, &msg, sizeof(msg), 0);
		if (nReadBytes == SOCKET_ERROR){
			// Nothing yet
		} else {
			if (msg.packet_type == PACKET_TYPE_AUTHENTICATE) {
				g->pl.name = g->logini.username_input;
				Fade(g, 0);
				g->menu.starting = true;
				NewGame(g);
				g->Interfaces[INTERFACE_MAIN_MENU].visible = false;

				g->Interfaces[INTERFACE_LOGIN].visible = false;
				g->logini.logging_in = false;
				g->connected = true;
			} else if (msg.packet_type == PACKET_TYPE_SERVER_MESSAGE) {
				ShowMessage(g, msg.packet_info_1, msg.packet_info_2);
			} else  if (msg.packet_type == PACKET_TYPE_CHAT_MESSAGE) {
				//
				AddChatMessage(g, msg.packet_info_1, 255, 255, 255, msg.packet_info_2);
			} else if (msg.packet_type == PACKET_TYPE_PLAYER_STATE) {
				int pid = atoi(msg.packet_info_1);
				std::istringstream args(msg.packet_info_2);
				args >> g->Players[pid].x;
				args >> g->Players[pid].y;
				args >> g->Players[pid].direction;
				args >> g->Players[pid].health;
				args >> g->Players[pid].sprite;
				args >> g->Players[pid].outfit;
				args >> g->Players[pid].hair;
				args >> g->Players[pid].acc;
				args >> g->Players[pid].frame;
				g->Players[pid].x_vel = msg.val1;
				g->Players[pid].y_vel = msg.val2;
			} else if (msg.packet_type == PACKET_TYPE_PLAYER_NAME) {
				int pid = atoi(msg.packet_info_1);
				std::istringstream args(msg.packet_info_2);
				args >> g->Players[pid].name;
			} else if (msg.packet_type == PACKET_TYPE_HIT) {
				std::istringstream args(msg.packet_info_2);
				int direction = 0;
				int dmg = 0;
				args >> direction;
				args >> dmg;
				g->pl.health -= dmg;
				g->pl.y_vel = -5;
				g->pl.x_vel = 5;
				if(direction == 1) g->pl.x_vel = -5;
				g->pl.combat_timer = SecondsToTicks(2.0);
			} else if (msg.packet_type == PACKET_TYPE_ACTION) {
				int pid = atoi(msg.packet_info_1);
				std::istringstream args(msg.packet_info_2);
				int action_type = -1;
				args >> action_type;
				if (action_type == ABILITY_ALPHASLASH) {
					if (msg.val1 == 76) {
						StartAnimation(g, 77, g->Players[pid].x - 72, g->Players[pid].y - 72, g->Players[pid].direction, true, 0);
						StartAnimation(g, 76, -72, -72, g->Players[pid].direction, false, 0, pid);
					} else {
						StartAnimation(g, 81, g->Players[pid].x - 72, g->Players[pid].y - 72, g->Players[pid].direction, true, 0);
						StartAnimation(g, 80, -72, -72, g->Players[pid].direction, false, 0, pid);
					}
				}
				if (action_type == ABILITY_BASIC_SPELL) {
					if (g->Players[pid].direction == 0) {
						CreateProjectile(g, ABILITY_BASIC_SPELL,
							g->Players[pid].x + g->Players[pid].w, g->Players[pid].y, g->Players[pid].direction);
					} else {
						CreateProjectile(g, ABILITY_BASIC_SPELL,
							g->Players[pid].x - 52, g->Players[pid].y, g->Players[pid].direction);
					}
				}
				if (action_type == 93) {
					// When another player admin-jumps
					g->Players[pid].y_vel = -10;
					g->Players[pid].colour = 3;
				}
				if (action_type == ABILITY_MEMEDASH) {
					// When another player memedashes
					g->Players[pid].x_vel = MAX_X_VEL * 3;
					if (g->Players[pid].direction == 1) g->Players[pid].x_vel = -MAX_X_VEL * 3;
					g->Players[pid].colour = 4;
					g->Players[pid].meme_dash_timer = SecondsToTicks(0.1);
				}
			} else if (msg.packet_type == PACKET_TYPE_PING) {
				g->ping = (int)((al_get_time() - g->time_sent_ping) * 1000);
			}
		}
		if (g->ReadyToSendPackets) {
			g->ReadyToSendPackets = false;
			for (int i = 0; i < 50; i++) {
				if (g->PacketQueue[i].packet_type != -1) {
					int nSendBytes = send_s(sock, &g->PacketQueue[i], sizeof(g->PacketQueue[i]), 0);
					// Ping
					if (g->PacketQueue[i].packet_type == PACKET_TYPE_PING) {
						g->time_sent_ping = al_get_time();
					}
					if (nSendBytes == SOCKET_ERROR) {
						std::cout << "Lost Connection to server\n";
						shutdown(sock, SD_SEND); // Shutdown our socket		
						closesocket(sock);		 // Close our socket entirely
						ShowMessage(g, "Lost connection to server.", "Get memed kid");
						g->scene = 0;
						HideAllInterfaces(g, 99);
						g->Interfaces[INTERFACE_MAIN_MENU].visible = true;
						g->Buttons[247].visible = true;
						g->connected = false;
						return 0;
					}
					g->PacketQueue[i].packet_type = -1;
				}
			}
		}
	}
	return 1;
}