...
void DrawImage(Game* g, ALLEGRO_BITMAP* img, int x, int y, int flags, float opacity){
	if (!g->done) {
		...
		if (img != NULL) {
			...
			if (collide(0, 0, GAME_WIDTH, GAME_HEIGHT, x, y, w, h)) {
				al_draw_tinted_scaled_bitmap(img, al_map_rgba_f(opacity, opacity, opacity, opacity), 0, 0, w, h, x*r_x, y*r_y, w*r_x, h*r_y, flags);
			}
		} else {
...
void DrawScaledImage(Game* g, ALLEGRO_BITMAP* img, int x, int y, int wi, int hi, int flags, float opacity) {
	if (!g->done) {
		...
		if (img != NULL) {
			...
			int h = al_get_bitmap_height(img);
			if (collide(0, 0, GAME_WIDTH, GAME_HEIGHT, x, y, w, h)) {
				al_draw_tinted_scaled_bitmap(img, al_map_rgba_f(opacity, opacity, opacity, opacity), 0, 0, w, h, x*r_x, y*r_y, (w+wi)*r_x, (h+hi)*r_y, flags);
			}
		} else {
			al_show_native_message_box(NULL, "Error", "File(s) Missing", "Game will quit.", "Quit", 0);
			g->done = true;
